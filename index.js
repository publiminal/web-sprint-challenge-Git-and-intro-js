...
function getArtistByIndex(artistArr, index) {
  return `the artist at index ${index} is ${artistArr[index].name}`;
}
...
function artistInfo(arr, name){
  const artist = arr.find(artist => artist.name === name);
...
function artistByCountry(arr, nation){
  //const artByCountry = arr.map(country => country.nationality === nation ? country );
  const artByCountry = [];
  for(let i=0; i<arr.length; i++){
      let artistMatchNation = arr[i].nationality === nation ;
      if(artistMatchNation){
        artByCountry.push(arr[i].name);
      }
  } 
  return artByCountry;    
}
